...
export const drawDotPatternToPdf = async (
  ...
  const rgbColor =
    colorToRgb(settings.patternColor) ??
    colorToRgb(defaultSettings.patternColor);
  if (rgbColor) {
  ...
  for (
    let yMm = firstDotY;
    yMm <= lastDotY + Number.EPSILON;
    yMm += dotSpacingMm
  ) {
    for (
      let xMm = firstDotX;
      xMm <= lastDotX + Number.EPSILON;
      xMm += dotSpacingMm
    ) {
